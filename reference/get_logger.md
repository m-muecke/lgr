...
#> WARN  [06:54:39.600] a warning message
lg
...
#> WARN  [06:54:39.606] a warning message
...
#> Warning: [2026-06-03 06:54:39.610] log/ger ~ error in `lg$info("lg has been invalidated an no longer works")`: Trying to log via a Logger reference that is no longer valid. Logger references become invalid when you reset a Logger with `get_logger(reset = TRUE)`. Please re-create the Logger reference with `get_logger(log/ger)`

lg <- get_logger("log/ger")
lg$info("now all is well again")
#> INFO  [06:54:39.612] now all is well again
```
