...
#> WARN  [09:59:53.676] a warning message
lg
...
#> WARN  [09:59:53.681] a warning message
...
#> Warning: [2026-06-02 09:59:53.685] log/ger ~ error in `lg$info("lg has been invalidated an no longer works")`: Trying to log via a Logger reference that is no longer valid. Logger references become invalid when you reset a when you reset a Logger with `get_logger(reset = TRUE)`. Please re-create the Logger reference with with `get_logger(log/ger)`

lg <- get_logger("log/ger")
lg$info("now all is well again")
#> INFO  [09:59:53.686] now all is well again
```
